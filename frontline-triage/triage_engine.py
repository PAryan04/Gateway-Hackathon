...
Uses Google Gemini API (free tier) to classify customer messages.
Features:
...
# ── Gemini Pricing (gemini-2.0-flash free tier = $0 up to quota) ─────────────
# Paid tier prices listed for reference if you exceed free quota:
...
def triage(
    ...
    if not api_key:
        ...
        return _safe_fallback("GEMINI_API_KEY not configured", input_format)
    ...
    for attempt in range(2):  # Attempt 0: normal, Attempt 1: stricter prompt
        try:
            ...
            # On retry, prepend a stricter instruction to the user message
            retry_prefix = ""
            if attempt == 1:
            ...
            if parsed is None:
                ...
                if attempt == 0:
                    continue  # Retry
                return _safe_fallback("LLM response not parseable after retry", input_format)
            ...
            if not _validate_schema(parsed):
                ...
                if attempt == 0:
                    continue  # Retry
                return _safe_fallback("Schema validation failed after retry", input_format)
            ...
            if "API_KEY_INVALID" in err_msg or "API key not valid" in err_msg:
                logger.error(f"[{message_id}] Gemini auth failed — check GEMINI_API_KEY")
                return _safe_fallback("Authentication error — invalid API key", input_format)
            ...
            if "RESOURCE_EXHAUSTED" in err_msg or "quota" in err_msg.lower():
                ...
                return _safe_fallback("Rate limit / quota exceeded", input_format)

            if "SAFETY" in err_msg or "blocked" in err_msg.lower():
                # Safety filter triggered — treat as gibberish needing human review
                logger.warning(f"[{message_id}] Gemini safety filter triggered")
                return {
                    "category": "general",
                    "priority": "P2",
                    "summary": "Message was flagged by content safety filters and could not be processed automatically.",
                    "suggested_action": "Route to human agent for manual review of flagged content.",
                    "needs_human": True,
                    "confidence": 0.1,
                    "language_detected": "und",
                    "input_format_detected": input_format,
            ...
            if attempt == 0:
                continue  # Retry once
            return _safe_fallback(f"Unexpected error: {err_type}", input_format)

    # Should never reach here, but belt-and-suspenders
    return _safe_fallback("All attempts exhausted", input_format)
